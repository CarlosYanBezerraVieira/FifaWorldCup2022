...
class FindStickerServiceImpl implements FindStickerService {
  ...
  Future<StickerModel> execute(String countryCode, String stickerNumber) async {
    var sticker = await stickersRepository.findStickerByCode(
        stickerCode: countryCode, stickerNumber: stickerNumber);
    if (sticker == null) {
      final registerStickerModel = RegisterStickerModel(
        stickerCode: countryCode,
        name: "",
        stickerNumber: stickerNumber,
      );
      sticker = await stickersRepository.create(
        registerStickerModel: registerStickerModel,
      );
    }
    return sticker;
  }
